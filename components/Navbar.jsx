  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-30 h-16 shadow-xl">
      ...
      <Transition
        ...
        show={showMenu}
      >
        <div className="sm:hidden z-10 bg-white shadow-xl pb-5 flex flex-col container space-y-3">
          {menu.map((item, key) => {
            return (
              <Link key={key} href={item.link}>
                <a
                  className={` ${
                    item.cta
                      ? "bg-blue-500 rounded-full mt-4 text-white px-5 py-2 font-bold text-center"
                      : "text-gray-800"
                  }`}
                >
                  {item.title}
